...
uint16_t ltdc_lcd_framebuf0[ACTIVE_HEIGHT][ACTIVE_WIDTH] __attribute__((at(LCD_FRAME_BUF_ADDR)));                 //图层0
uint16_t ltdc_lcd_framebuf1[ACTIVE_HEIGHT][ACTIVE_WIDTH] __attribute__((at(LCD_FRAME_BUF_ADDR + 800 * 480 * 2))); //图层1
...
//#define LCD_WR_REG(x)   Write_3Spi_cmd(x)
//#define LCD_WR_DATA(x)  Write_3Spi_data(x)
...
void NT35510_Init(void)
{
    ...
    delay_1ms(300);
		...
		LCD_WR_REG(0X36);//Set_address_mode.XY,rgb.bgr横竖屏
		LCD_WR_DATA(0X28);// 
    ...
    int b_VERTICAL_FRONT_PORCH)
{
  ...
  /* initialize the parameters of structure */
    ...
    rcu_tli_clock_div_config(RCU_PLLSAIR_DIV2);
      ...
      rcu_tli_clock_div_config(RCU_PLLSAIR_DIV4);           //PLLSAIR 48MHz, lcd need 11Mhz
...
#define LCD_DrawPoint(x, y, color)  LTDC_Draw_Point(1, x, y, color)

...
#define LCD_SDA_WRITE(x)	    do{gpio_bit_write(LCD_SDA_PORT,LCD_SDA_PIN,x);}while(0)
#define LCD_SCL_RCU  			RCU_GPIOE   // SCL
#define LCD_SCL_PORT  		    GPIOE
#define LCD_SCL_PIN    		    GPIO_PIN_2
#define LCD_SCL_WRITE(x)	    do{gpio_bit_write(LCD_SCL_PORT,LCD_SCL_PIN,x);}while(0)
#define LCD_CS_RCU  			RCU_GPIOE   // CS
#define LCD_CS_PORT  		    GPIOE
#define LCD_CS_PIN    		    GPIO_PIN_4
#define LCD_CS_WRITE(x)	        do{gpio_bit_write(LCD_CS_PORT,LCD_CS_PIN,x);}while(0)
#define LCD_RST_RCU  			RCU_GPIOD   // RST
#define LCD_RST_PORT  		    GPIOD
#define LCD_RST_PIN    		    GPIO_PIN_12
#define LCD_RST_WRITE(x)	    do{gpio_bit_write(LCD_RST_PORT,LCD_RST_PIN,x);}while(0)
...
#define LCD_SDI_WRITE(x)	    do{gpio_bit_write(LCD_SDI_PORT,LCD_SDI_PIN,x);}while(0)


#define HORIZONTAL_SYNCHRONOUS_PULSE  10
#define HORIZONTAL_BACK_PORCH         150
#define ACTIVE_WIDTH                  300
#define HORIZONTAL_FRONT_PORCH        20

#define VERTICAL_SYNCHRONOUS_PULSE    10
#define VERTICAL_BACK_PORCH           140
#define ACTIVE_HEIGHT                 300                                                                   
#define VERTICAL_FRONT_PORCH          15


extern uint16_t *ltdc_framebuf[2];              
extern uint16_t ltdc_lcd_framebuf0[ACTIVE_HEIGHT][ACTIVE_WIDTH];              
extern uint16_t ltdc_lcd_framebuf1[ACTIVE_HEIGHT][ACTIVE_WIDTH]; 
...
void LCD_ShowPicture(uint8_t layer, uint16_t x, uint16_t y, uint16_t length, uint16_t width, uint16_t *data);
